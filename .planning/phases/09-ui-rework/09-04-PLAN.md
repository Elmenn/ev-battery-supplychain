...
must_haves:
  truths:
    ...
    - "Seller can select transporter with fee deposit confirmation modal"
    - "Buyer sees 'Buy with Railgun' button when phase is Listed"
    - "Buyer sees payment recorded card with truncated memoHash/txRef after purchase"
...
- `getPrivatePaymentHistoryKey`, `getPendingPrivatePaymentKey`
  ...
  <done>ProductDetail.jsx gutted of all dead code, uses getProductState/detectRole from escrowHelpers, has clean state management.</done>
</task>
...
const handleConfirmOrder = async () => {
  ...
  try {
    ...
    const updatedVc = appendPaymentProof(listingVc, {
      ...
      memoHash: "0x" + "0".repeat(64), // placeholder -- buyer's memo is on-chain
      railgunTxRef: "0x" + "0".repeat(64), // placeholder
...
2. `handleSelectTransporter(bidAddress)`:
```javascript
const handleSelectTransporter = async (bidAddress) => {
  setActionLoading(true);
  try {
    const signer = await provider.getSigner();
    const contract = getEscrowContract(address, signer);
    const fee = await contract.transporters(bidAddress);

    // Confirm fee deposit
    if (!window.confirm(`You will deposit ${ethers.formatEther(fee)} ETH as delivery fee. Continue?`)) {
      setActionLoading(false);
      return;
    }

    const tx = await contract.setTransporter(bidAddress, { value: fee });
    await tx.wait();
...
return (
  <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
    ...
    {/* Show seller window when phase=Purchased */}
    ...
    {/* Show bid window when phase=OrderConfirmed */}
    ...
    {/* Show delivery window when phase=Bound */}
                ...
                .map((bid, i) => (
                  <tr key={bid.address} className="border-b">
                    ...
                    <td className="py-2 text-right">
                      <Button size="sm" onClick={() => handleSelectTransporter(bid.address)} disabled={actionLoading}>
                        Select
                      </Button>
                    </td>
                  </tr>
    ...
    {/* BUYER: Post-purchase info (phase >= Purchased, role=buyer) */}
    {role.role === "buyer" && product.phase >= Phase.Purchased && (
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        ...
        <p className="text-sm text-green-700">Your payment has been recorded on-chain.</p>
      </div>
    )}

    {/* HASH DISPLAY (phase >= OrderConfirmed, seller/buyer/transporter) */}
    {product.phase >= Phase.OrderConfirmed && product.vcHash && product.vcHash !== "0x" + "0".repeat(64) && (
...
Note: The transporter action panel (bid submission, delivery confirmation) is handled in Plan 05. This plan focuses on seller + buyer + shared display. Leave a comment `{/* Transporter actions -- see Plan 05 */}` where the transporter panel will go.
  ...
  <verify>
    ...
    - Seller confirm order handler calls appendPaymentProof + signVcAsSeller + uploadJson + confirmOrder
    - Seller transporter selection handler calls setTransporter with fee as msg.value
    - Buyer sees "Buy with Railgun" button when phase is Listed
  ...
  <done>ProductDetail.jsx rewritten: ~600-800 lines (down from 2100), uses escrowHelpers for all contract interaction, role-aware panels for seller (confirm order, select transporter) and buyer (buy, post-purchase display), shared PhaseTimeline + HashDisplay + CountdownTimer. All dead code from old flow removed.</done>
</task>
...
- Seller confirm order creates v2.0 VC (appendPaymentProof)
...
- Seller confirm is 1-click: VC sign + IPFS upload + on-chain confirmOrder
- Seller sees sortable bid table with "Lowest" badge when phase is OrderConfirmed
- Seller can select transporter with fee deposit confirmation
- Buyer sees "Buy with Railgun" when phase is Listed
- Buyer sees "Purchase Complete" card after payment
