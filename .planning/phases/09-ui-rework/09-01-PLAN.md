...
must_haves:
  truths:
    ...
    - "getProductState reads id, productMemoHashes(id), and productRailgunTxRefs(id) from contract"
...
2. Create `frontend/src/utils/escrowHelpers.js` with:
   ...
   - `Phase` enum object mapping phase names to numbers:
     ...
     ```
     IMPORTANT: There are exactly 6 phases. There is NO "Slashed" phase. Slashing is an event
     that occurs within the Expired state (bonds are forfeited on timeout). Do NOT add a Slashed
     entry to the Phase enum.
   ...
   - `PHASE_LABELS` mapping Phase values to display strings:
     ...
     ```
     Again: NO "Slashed" label. Only these 6 labels.
   ...
   - `getProductState(address, provider)` -- reads ALL contract state in parallel via Promise.all:
     ...
     CRITICAL: After the initial parallel read, use the returned `id` value to read:
     - `productMemoHashes(id)` -> stored as `memoHash` (bytes32)
     - `productRailgunTxRefs(id)` -> stored as `railgunTxRef` (bytes32)

     These are mappings (uint256 => bytes32) on the contract, so they must be called with the product's id.
     The second read can be a separate Promise.all after id is available, or included in the first
     batch if id is known. Since id is read from contract, do a two-step read:

     ```javascript
     // Step 1: Read all scalar fields including id
     const [name, owner, buyer, purchased, delivered, transporter, phase, vcHash,
            priceCommitment, sellerBond, bondAmount, deliveryFee,
            purchaseTimestamp, orderConfirmedTimestamp, boundTimestamp, productId] =
       await Promise.all([
         contract.name(), contract.owner(), contract.buyer(), contract.purchased(),
         contract.delivered(), contract.transporter(), contract.phase(), contract.getVcHash(),
         contract.priceCommitment(), contract.sellerBond(), contract.bondAmount(),
         contract.deliveryFee(), contract.purchaseTimestamp(), contract.orderConfirmedTimestamp(),
         contract.boundTimestamp(), contract.id()
       ]);
     ...
     Returns object with all fields including `id` (as Number), `memoHash`, `railgunTxRef`,
     phase as Number, timestamps as Number, address included.
   ...
   IMPORT PATH: Import ABI from `../abis/ProductEscrow_Initializer.json` (use `.abi` property). Import ethers from "ethers".
   The file is at `src/utils/escrowHelpers.js` and ABIs are at `src/abis/`, so the relative path is `../abis/...` (one level up).
  ...
  <verify>
    ...
    - Exports: Phase, PHASE_LABELS, SELLER_WINDOW, BID_WINDOW, DELIVERY_WINDOW, getEscrowContract, getProductState, detectRole
    - `grep "productMemoHashes" frontend/src/utils/escrowHelpers.js` returns match
    - `grep "productRailgunTxRefs" frontend/src/utils/escrowHelpers.js` returns match
    - `grep "contract.id()" frontend/src/utils/escrowHelpers.js` returns match
    - `grep "../abis/ProductEscrow_Initializer" frontend/src/utils/escrowHelpers.js` returns match (correct relative path)
  ...
  <done>escrowHelpers.js exports all contract interaction functions and constants. getProductState reads id, memoHash, and railgunTxRef from contract. No scattered contract instantiation needed in components. ABI import path is correct (../abis/).</done>
</task>
...
- The contract enum has exactly 6 phases: Listed(0), Purchased(1), OrderConfirmed(2), Bound(3), Delivered(4), Expired(5).
  There is NO "Slashed" phase. Do NOT render "Slashed" as a timeline step. Slashing is an event/penalty
  that happens within the Expired state, not a separate phase.
- Desktop (md+ breakpoint): horizontal stepper with circles connected by lines. Current phase highlighted in blue, completed phases in green, future phases in gray. Expired shown in red.
...
All components use Tailwind CSS classes exclusively. No inline styles. Import lucide-react icons where needed (Shield for BondCard).
  ...
  <verify>
    ...
    - PhaseTimeline does NOT contain "Slashed" as a phase step or label
    - `cd frontend && npx react-scripts build 2>&1 | head -20` -- no import/syntax errors
  </verify>
  <done>Four shared components created: PhaseTimeline (adaptive horizontal/vertical, 6 phases only -- no "Slashed" step), HashDisplay (copy + QR), CountdownTimer (live with color thresholds), BondCard (read-only bond display). All use Tailwind CSS and are ready for composition in ProductDetail and modals.</done>
</task>
...
- escrowHelpers.js Phase enum matches contract Phase enum (0-5, exactly 6 values, no "Slashed")
- escrowHelpers.js ABI import uses `../abis/...` (correct path from src/utils/ to src/abis/)
- getProductState returns memoHash and railgunTxRef fields read from productMemoHashes(id) and productRailgunTxRefs(id)
</verification>

<success_criteria>
- escrowHelpers.js provides centralized contract access (no more scattered ethers.Contract calls needed)
- getProductState reads id, productMemoHashes(id), productRailgunTxRefs(id) and returns them as memoHash/railgunTxRef
...
- PhaseTimeline shows exactly 6 phases (no "Slashed" step)
</success_criteria>
