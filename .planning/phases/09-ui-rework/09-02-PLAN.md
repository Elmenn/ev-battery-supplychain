...
must_haves:
  truths:
    - "MarketplaceView uses getProductState from escrowHelpers instead of scattered contract calls"
...
<objective>
Update MarketplaceView and ProductCard to use escrowHelpers for contract reads, remove dead publicPriceWei reads, add transporter-oriented filter tabs, and implement dual-badge display on cards.
...
1. Import `getProductState` and `getEscrowContract` from `../../utils/escrowHelpers`.
2. REMOVE the `publicPriceWei` read -- this getter no longer exists on the contract. It will throw.
3. For each product address, call `getProductState(address, provider)` to get all fields in one call (this returns phase, bondAmount, deliveryFee, sellerBond, memoHash, railgunTxRef, id, etc.).
4. Set `price: "Private"` for all products (private-only marketplace).
5. For transporter filter, also read: `contract.getAllTransporters()` via `getEscrowContract(address, provider)` -- returns [addresses[], fees[]]. Store as `transporterAddresses` and `transporterFees` arrays on the product object.
6. Remove the `publicPriceWei` field from the product object entirely.
...
- Remove `priceBlinding` localStorage read (not used in new flow).
  ...
  <verify>
    ...
    - `grep "getProductState\|escrowHelpers" frontend/src/views/MarketplaceView.jsx` returns matches
  </verify>
  <done>MarketplaceView loads product state via getProductState from escrowHelpers, removes publicPriceWei dead code, and shows 6 filter tabs including transporter discovery filters.</done>
</task>
...
<verification>
- MarketplaceView uses getProductState from escrowHelpers for all contract reads
