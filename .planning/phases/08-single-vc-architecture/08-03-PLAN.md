...
- If `vc.credentialSubject.price` exists instead of priceCommitment (v1.0 pattern), accept as valid but note in errors: "WARNING: v1.0 price field detected (backward compatible)"
- `vc.proof` is an array (can be empty for unsigned VCs)

`valid` is true if no errors (warnings prefixed with "WARNING: " don't count).
...
- `onChainCommitment` (string): The bytes32 commitment from contract (hex string, may or may not have 0x prefix)
...
- Pass BOTH values directly to `verifyCommitmentMatch(vcCommitment, onChainCommitment)` -- it already handles 0x prefix stripping and lowercase normalization internally, so do NOT strip 0x prefix before calling it
- Return result with both commitment values for transparency
...
Should show all 5 exports, schema valid: true, proofs valid: true with 1 proof, overall: true.
  ...
  - verifyPriceCommitment passes values directly to verifyCommitmentMatch (which handles 0x normalization internally)
...
- verifyPriceCommitment does NOT strip 0x prefix before calling verifyCommitmentMatch (it handles normalization)
