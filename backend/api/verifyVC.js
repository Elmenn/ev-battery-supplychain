...
const VC_SIGN_PAYLOAD_FORMAT_V4_ERC7984_TYPED = 'eip712-v4-erc7984-vrc-typed';
...
function preparePayloadForVerification(vc, payloadFormat) {
  if (payloadFormat === VC_SIGN_PAYLOAD_FORMAT_V4_ERC7984_TYPED) {
...
async function verifyVC(vcJsonData, contractAddress = null) {
  ...
  const payloadFormat =
    issuerProof?.payloadFormat ||
    (
      (String(vcJsonData?.schemaVersion || '') === '6.0'
        || String(vcJsonData?.schemaVersion || '') === '6.1')
      && vcJsonData?.credentialSchema
      && vcJsonData?.credentialStatus
      && vcJsonData?.credentialSubject?.paymentBridge
