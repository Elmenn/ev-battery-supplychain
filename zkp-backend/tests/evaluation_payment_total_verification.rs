...
//! `payment = total`
//!
//! This test measures proof verification time for both proof families using the
//! same committed inputs and the same binding context.

use bulletproof_demo::zk::bp_payment_total_proof::{
    prove_payment_total_equality, verify_payment_total_equality,
};
...
mod tests {
    ...
    #[test]
    fn compare_payment_total_verification_time() {
        println!("\n🧪 Fiat-Shamir vs Bulletproof: payment = total proof verification\n");
        ...
        assert!(verify_payment_total_equality(c_total, c_pay, &bp_proof, &context));
        ...
        for i in 0..RUNS {
            ...
            let bp_verified = verify_payment_total_equality(c_total, c_pay, &bp_proof, &context);
