...
use bulletproof_demo::zk::bp_quantity_total_proof::prove_quantity_total_bulletproof_raw;
...
mod tests {
    ...
    #[test]
    fn compare_quantity_total_generation_time() {
        println!("\n🧪 Fiat-Shamir vs Bulletproof: total = unitPrice * quantity proof generation\n");

        const RUNS: usize = 100;
        let (c_quantity, c_total, quantity_value, total_value, unit_price, r_quantity, r_total, context) =
            build_shared_fixture();

        let _ = prove_quantity_total(
            c_quantity,
            c_total,
            unit_price,
            r_quantity,
        ...
        .unwrap();
        let _ = prove_quantity_total_bulletproof_raw(
            c_quantity,
            c_total,
            quantity_value,
            total_value,
        ...
        for i in 0..RUNS {
            ...
            .expect("Fiat-Shamir quantity-total generation should succeed");
            fs_times.push(fs_start.elapsed().as_nanos() as f64 / 1_000_000.0);

            let bp_start = Instant::now();
            let bp_proof = prove_quantity_total_bulletproof_raw(
                c_quantity,
                c_total,
                quantity_value,
                total_value,
            ...
            .expect("Bulletproof quantity-total generation should succeed");
