...
export async function saveOrderAttestation({
  orderId,
  productAddress,
  buyerAddress,
  encryptedBlob,
  ...
  const res = await fetch(`${BACKEND_URL}/order-attestations`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      orderId: String(orderId).toLowerCase(),
      productAddress: productAddress.toLowerCase(),
      buyerAddress: buyerAddress.toLowerCase(),
      encryptedBlob,
      disclosurePubkey,
      encryptedQuantityOpening: encryptedQuantityOpening || null,
      encryptedTotalOpening: encryptedTotalOpening || null,
      quantityTotalProof: quantityTotalProof || null,
      paymentEqualityProof: paymentEqualityProof || null,
      proofBundle: proofBundle || null,
    }),
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(body.error || `saveOrderAttestation failed: ${res.status}`);
  }
  return res.json();
}

export async function getOrderAttestation(orderId) {
  try {
    const res = await fetch(`${BACKEND_URL}/order-attestations/${String(orderId).toLowerCase()}`);
    if (res.status === 404) return null;
    if (!res.ok) {
      console.warn(`getOrderAttestation: unexpected status ${res.status}`);
      return null;
    }
    return res.json();
  } catch (err) {
    console.warn('getOrderAttestation: network error', err.message);
    return null;
  }
}

export async function updateOrderProofBundle(orderId, {
  encryptedQuantityOpening,
  encryptedTotalOpening,
  quantityTotalProof,
  paymentEqualityProof,
  ...
  const res = await fetch(
    `${BACKEND_URL}/order-attestations/${String(orderId).toLowerCase()}/proof-bundle`,
    {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(body.error || `updateOrderProofBundle failed: ${res.status}`);
  }
  return res.json();
}
