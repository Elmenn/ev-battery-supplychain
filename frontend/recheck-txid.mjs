// file: recheck-txid.mjs
import {
  ChainType,
  NetworkName,
  NETWORK_CONFIG,
  TXIDVersion,
  networkForChain,          // ← use this to build the "network" param
...
async function probe(name, id) {
  ...
  const network = networkForChain(ChainType.EVM, id); // ✅ correct param
...
(async () => {
  for (const [name, id] of CHAINS) await probe(name, id);
})();
